...
class GetCalls {
  final String ip;
  String baseUrl;
  GetCalls({required this.ip}) : baseUrl = 'http://$ip:5505';
  ...
  Future<List<ShowDetailsEntity>> getAllShows() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        debugPrint(
          'Successful get_shows command. Response is ${response.body}',
        );
  ...
  List<AllProjectsModel> allProjectsModelList(
    ...
    if (projectEntry.used != 0) {
      ...
      List<ShowDetailsEntity> filteredShows =
          allShows
              .where((show) => projectShowIds.contains(show.showId))
              .toList();
  ...
  Future<void> getSlideThumbnail(
    ...
    if (response.statusCode == 200) {
      ...
      // Handle success with no content
      debugPrint(
        'Get get_slide_thumbnail was successful, no content returned.',
      );
    } else {
      debugPrint(
